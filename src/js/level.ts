...
PIXI.loader.load(border);
PIXI.loader.load(levelSettings)
levelSettings();
  function border(){
    for (var i = 0; i < 144; i++) {
      const levelDust = new PIXI.Sprite(imgs.dustImg);
...
export function grassMap(count:number){
  for(var i = 0; i < 10*count; i++) {
    const grassMap = new PIXI.Sprite(imgs.grassImg);
...
export function asphaltMap(count:number){
  for(var i = 0; i < 10*count; i++) {
    const asphaltMap = new PIXI.Sprite(imgs.asphaltImg);
...
export function oceanMap(count:number){
  for(var i = 0; i < 10*count; i++) {
    const oceanMap = new PIXI.Sprite(imgs.oceanImg);
